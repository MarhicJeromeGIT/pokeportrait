...
function findHoveredWord(ev)
{
	...
	var hoveredNode = range.startContainer;
	...
	var currentWord = "";
	for( var i=0; i< hoveredNode.nodeValue.length; i++ )
	{		
		if( inWord ) // look for a space to end the word
		{
			if( hoveredNode.nodeValue[i].match(/\s/i) )
			{
				// If we were reading the word on which the cursor (carret) is on,
				// We should break now
				if( i >= offset )
				{
					break;
			...
			{
				currentWord += hoveredNode.nodeValue[i];// add the letter to the word
			}
		}
		else // look for a letter to begin the word
		{
			if( hoveredNode.nodeValue[i].match(/[a-zA-Z]/i) )
			{
				inWord = true;
				wordStartIndex = i;
				currentWord += hoveredNode.nodeValue[i]; // add the letter to the word (first letter)
			}
		}
	}
	// So what was the word ?
	//console.log("hovered word : "+currentWord);
	return currentWord;
}
...
function showDialog( pokindex )
{
	...
	{
		var imgUrl =  chrome.extension.getURL("./pokemon/dream-world/"+nationalPokedexNumber+".svg");
	}
	else
	{
...
function onMouseMoved(ev)
{
	...
	if( pokindex > -1 )
	{
		...
		if( !currentlyShowing || currentPokemon != pokindex )
		{
			showDialog( pokindex );
...
function getPokedex()
{
	...
	for( var i=0 ; i< nameInfo.length; i++ )
	{
		...
		var pokeName = { natIndex	: idx,
						 ...
						 typeI		: pokeTypes[idx].typeI,
						 typeII		: pokeTypes[idx].typeII,
